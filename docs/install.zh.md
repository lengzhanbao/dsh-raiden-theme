...
- [ ] 页面出现花房 / 霓虹舞台壁纸与粉金对话框
- [ ] **设置 → 通用** 里能看到 Raiden 滑块（背景纱、面板透明度等）
...
- 只想要界面 → 开 **雷电模式** 即可
- 想要雷电将军人设 → 在 **Agent 预设** 里另选 **「Raiden 雷电将军」**
...
浏览器硬刷新即可。`localStorage` 里的 Raiden 设置会保留。
...
在 **雷电模式** 里把 **背景纱** 调到 14%–18%，或略提高 **面板透明度**。深色下可开启 **减弱动效**。

### 和其他插件叠在一起发灰 / 透明

本主题亚克力为 **按需启用**，不会全局磨砂第三方插件。若某插件仍异常，请 [提 Issue](https://github.com/lengzhanbao/dsh-raiden-theme/issues) 并说明插件名与 DSH 版本。
