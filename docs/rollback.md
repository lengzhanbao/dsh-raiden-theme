...
| 阶段 | 回退方式 |
|------|----------|
| P1 静态门禁 | 删除 `scripts/verify-raiden.mjs`、`verify/`，从 `package.json` 去掉 `verify:*` / `prepublishOnly` |
| P2 资产门控 | 删除 `scripts/verify-assets.mjs`、`scripts/asset-metrics.py`、`assets/raiden/baseline.json`；去掉 `prebuild` |
...
1. 从 git 恢复旧图：`git checkout -- assets/raiden/left-light.webp assets/raiden/left-light.png`
2. 或调高 `assets/raiden/baseline.json` 里 `darkEdgeRatioMax`（不推荐）
