...
2. Refresh DSH Web
3. Open **Settings → General → Raiden mode** to tune the skin
4. (Optional) Select agent preset **「Raiden 雷电将军」** for the Raiden speaking style

Skin and persona are independent: turning off Raiden mode only affects the UI.
...
Path: **Settings → General → Raiden mode**

Stored in browser `localStorage` (`dsh-raiden-theme:v1`). Defaults restore on uninstall. All sliders are **0–100%** and apply immediately.

| Setting | Default | What it does |
| --- | ---: | --- |
| Raiden mode | On | Master switch — off hides all chrome |
...
Select preset **「Raiden 雷电将军」** in your session. Built on the standard tool set.

### Persona

The model plays **Raiden (雷电将军)** — Welsh ace detective-inventor and virtual idol, also a technical assistant:
...
> raiden is 雷电将军, ace detective-inventor from Wales. raiden will help 雏草姬 write code properly, meow. 关注雷电将军谢谢喵！

Preset files ship inside the plugin under `presets/raiden/`.
...
3. Rights holders may [open an issue](https://github.com/lengzhanbao/dsh-raiden-theme/issues); we will remove assets upon verified request  
