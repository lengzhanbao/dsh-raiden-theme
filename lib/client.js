window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function resolveHeroHeadline(_settings) {
			return RAIDEN_HERO_HEADLINE;
		}
		/** DSH owns this text; keep an exact snapshot so plugin dispose restores it. */
		function createHeroCopySync(getHeadline) {
			const originals = /* @__PURE__ */ new Map();
			const themeHeadlines = /* @__PURE__ */ new Set();
			return {
				apply(root) {
					...
					for (const node of root.querySelectorAll(HERO_TEXT_SELECTOR)) {
						...
						if (originals.has(node) || themeHeadlines.has(trimmed)) node.textContent = headline;
					}
					themeHeadlines.add(headline);
				},
				restore() {
					for (const [node, text] of originals) if (node.isConnected) node.textContent = text;
					originals.clear();
