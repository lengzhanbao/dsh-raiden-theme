...
function clearOwnedSurface(element) {
    if (element.getAttribute(OWNER_ATTR) !== 'dsh-raiden-theme')
        return;
    element.removeAttribute(OWNER_ATTR);
    element.removeAttribute(SURFACE_ATTR);
}
function clearOwnedSurfaces(doc) {
    for (const node of doc.querySelectorAll(`[${OWNER_ATTR}="dsh-raiden-theme"]`)) {
        if (node instanceof HTMLElement)
            clearOwnedSurface(node);
    }
}
/**
 ...
 */
export function startAcrylicSurfaces(doc) {
    clearOwnedSurfaces(doc);
    return () => {
        clearOwnedSurfaces(doc);
    };
}
//# sourceMappingURL=acrylic-surfaces.js.map