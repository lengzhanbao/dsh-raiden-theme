import { RIGHT_DOCK_SELECTOR, SIDEBAR_SELECTOR, SKIN_OWNER } from './chrome-selectors';
function measureRightPanelWidth(doc) {
    ...
    for (const node of doc.querySelectorAll(RIGHT_DOCK_SELECTOR)) {
        if (!(node instanceof HTMLElement))
            continue;
        if (node.closest(`[data-skin-owner="${SKIN_OWNER}"]`))
            continue;
        const box = node.getBoundingClientRect();
        if (box.width < 160 || box.height < 120)
            continue;
        if (box.left < vw * 0.5)
            continue;
        if (box.left >= vw - 12)
...
export function startSidebarMetrics(doc) {
    ...
    const writeFooter = (sidebar) => {
        ...
        if (parent instanceof HTMLElement && parent !== sidebar) {
            ...
            if (parentBox.height > 0 && parentBox.height < sidebarBox.height * 0.35)
                cluster = parent;
        }
    ...
    const attachSidebar = (sidebar) => {
        ...
        sidebarObserver = new ResizeObserver((entries) => {
            ...
            if (entry)
                writeSidebar(entry.contentRect.width);
            writeFooter(sidebar);
        });
...
//# sourceMappingURL=sidebar-metrics.js.map