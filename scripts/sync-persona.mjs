/**
 * Inject src/prompt/raiden-system.md into presets/raiden/agent.cordis.yml persona.text.
 * Single source of truth for the Raiden agent persona.
 */
import fs from 'node:fs'
import path from 'node:path'

const root = process.cwd()
const prompt = fs.readFileSync(path.join(root, 'src/prompt/raiden-system.md'), 'utf8')
const agentPath = path.join(root, 'presets/raiden/agent.cordis.yml')
...
if (startIdx === -1 || endIdx === -1) {
  throw new Error('persona block markers not found in presets/raiden/agent.cordis.yml')
}
...
console.log('synced persona into presets/raiden/agent.cordis.yml')

const dshHome = process.env.DSH_HOME || 'E:/DeepSeekHarness'
const presetSrc = path.join(root, 'presets/raiden')
const presetDst = path.join(dshHome, '.agent-presets/raiden')
