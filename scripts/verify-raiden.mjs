/**
 * Raiden theme static release gate.
 *
 *   node scripts/verify-raiden.mjs              static half only
 *   node scripts/verify-raiden.mjs capture/*.json   optional live metrics (scene field required)
 */
import {
  existsSync,
  readFileSync,
...
const manifest = JSON.parse(readFileSync(join(ROOT, 'assets/raiden/manifest.json'), 'utf8'))
for (const figure of Object.values(manifest.figures ?? {})) {
  check(`manifest figure ${figure.file}`, existsSync(join(ROOT, 'assets/raiden', figure.file)))
}
for (const file of Object.values(manifest.wallpaper ?? {})) {
  check(`manifest wallpaper ${file}`, existsSync(join(ROOT, 'assets/raiden', file)))
}
