...
export function isNightScene(root: HTMLElement = document.body): boolean {
  return root.hasAttribute('data-ds-dark-theme')
    || root.getAttribute('data-raiden-preset') === 'raiden-night'
}
...
export function resolveAvatarUrl(settings: RaidenSettings, custom?: RaidenImageConfig, night = isNightScene()): string {
...
export function resolvePortraitUrl(settings: RaidenSettings, custom?: RaidenImageConfig): string | null {
...
export function resolveBannerUrl(settings: RaidenSettings, custom?: RaidenImageConfig): string | null {
...
export function resolveQChromeUrls(settings: RaidenSettings): {
  face: string
  send: string
  stop: string
  newSession: string
