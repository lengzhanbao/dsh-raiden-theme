...
function RaidenModeRow() {
  ...
  return createElement('div', {
    ...
    createElement('div', { className: 'dsh-raiden-general-cubes' },
      ...
      createElement(Cube, { selected: settings.showMascot, label: '鸣神', onClick: () => commit({ showMascot: !settings.showMascot }) }),
    ),
    createElement(Slider, {
      label: '立绘透明度',
    ...
    createElement('div', { className: 'dsh-raiden-general-note' },
      '工房开了喵：浅色花房、深色舞台跟着「外观」走。边框包住对话，百分比只改透明层，人物默认不透明。想听雷电将军说话，去 Agent 预设里选「Raiden 雷电将军」喵。',
    ),
  )
}
