...
import { DEFAULT_SETTINGS, parseRaidenSettings } from '../config'

export const STORAGE_KEY = 'dsh-raiden-theme:v1'
export const SETTINGS_CHANGE_EVENT = 'dsh-raiden-theme:settings-change'
...
export function veilBucket(settings: Pick<RaidenSettings, 'veilOpacity'>): LegacyVeil {
...
export function loadSettings(): RaidenSettings {
  try {
...
export function saveSettings(next: RaidenSettings): void {
  const clean = parseRaidenSettings(next)
  localStorage.setItem(STORAGE_KEY, JSON.stringify(clean))
  window.dispatchEvent(new CustomEvent(SETTINGS_CHANGE_EVENT, { detail: clean }))
}

export function subscribeSettings(onChange: (settings: RaidenSettings) => void): () => void {
