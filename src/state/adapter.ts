...
let lastState: RaidenAgentState = 'idle'
let lastChangeAt = 0
...
function scheduleFlash(next: 'success' | 'error', holdMs: number, onState: (s: RaidenAgentState) => void): void {
...
export function mapRuntimeState(input: unknown): RaidenAgentState {
...
export function mapDomSignals(signals: {
  ...
  let next: RaidenAgentState = 'idle'
...
export function debounceState(state: RaidenAgentState): RaidenAgentState {
  return state
}
