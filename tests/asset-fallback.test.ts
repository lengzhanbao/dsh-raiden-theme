...
describe('character asset recovery', () => {
  it('clears data-raiden-asset-error when src is restored', () => {
    const image = document.createElement('img')
    bindAssetFallback(image)
    image.dispatchEvent(new Event('error'))
    expect(image.hasAttribute('data-raiden-asset-error')).toBe(true)

    applyImageSrc(image, PIXEL)
    expect(image.hasAttribute('data-raiden-asset-error')).toBe(false)
    expect(image.getAttribute('src')).toBe(PIXEL)
  })

  it('clears data-raiden-asset-error on a successful load after error', () => {
    const image = document.createElement('img')
    bindAssetFallback(image)
    image.dispatchEvent(new Event('error'))
    expect(image.hasAttribute('data-raiden-asset-error')).toBe(true)
