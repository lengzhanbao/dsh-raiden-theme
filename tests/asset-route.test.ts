...
describe('plugin asset route', () => {
  it('rejects path traversal', () => {
    const root = mkdtempSync(join(tmpdir(), 'raiden-assets-'))
    expect(() => resolveAssetPath('../secret.txt', root)).toThrow(/escapes plugin root/)
  })

  it('serves files under the plugin assets prefix', async () => {
    const root = mkdtempSync(join(tmpdir(), 'raiden-assets-'))
    const raidenDir = join(root, 'raiden')
    mkdirSync(raidenDir, { recursive: true })
    writeFileSync(join(raidenDir, 'avatar.webp'), 'webp-bytes')
    ...
    await servePluginAsset(
      { method: 'GET', headers: { host: '127.0.0.1:3080' }, url: `${PLUGIN_ASSET_ROUTE_PREFIX}/raiden/avatar.webp` },
      res as never,
      `${PLUGIN_ASSET_ROUTE_PREFIX}/raiden/avatar.webp`,
      root,
    )
  ...
  it('rejects non-loopback requests', async () => {
    ...
    await servePluginAsset(
      { method: 'GET', headers: { host: '203.0.113.10:3080' }, url: `${PLUGIN_ASSET_ROUTE_PREFIX}/raiden/x.webp` },
      res as never,
      `${PLUGIN_ASSET_ROUTE_PREFIX}/raiden/x.webp`,
      root,
    )
