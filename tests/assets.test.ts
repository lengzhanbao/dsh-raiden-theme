...
describe('asset validation', () => {
  ...
  it('cache-busts bundled asset URLs so refresh is not required', () => {
    ...
    expect(url).toContain('/plugins/@dsh-external/dsh-raiden-theme/assets/raiden/wallpaper-light.webp')
    expect(url).toContain(`?v=${ASSET_SET_VERSION}`)
    expect(isAllowedImageProtocol(url)).toBe(true)
  })

  it('validates image protocols', () => {
    expect(isAllowedImageProtocol('/plugins/@dsh-external/dsh-raiden-theme/assets/raiden/generated/avatar.webp')).toBe(true)
