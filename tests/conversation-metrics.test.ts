...
describe('conversation metrics', () => {
  afterEach(() => {
    ...
    document.body.removeAttribute('data-raiden-frame-hidden')
    document.body.removeAttribute('data-raiden-frame-compact')
    vi.unstubAllGlobals()
    vi.restoreAllMocks()
  })
  ...
  it('writes the real conversation rect onto body CSS variables', () => {
    ...
    expect(document.body.style.getPropertyValue('--raiden-conversation-left')).toBe('280px')
    expect(document.body.style.getPropertyValue('--raiden-conversation-top')).toBe('16px')
    expect(document.body.style.getPropertyValue('--raiden-conversation-width')).toBe('720px')
    expect(document.body.style.getPropertyValue('--raiden-conversation-height')).toBe('640px')
    expect(document.body.hasAttribute('data-raiden-frame-hidden')).toBe(false)
    expect(document.body.hasAttribute('data-raiden-frame-compact')).toBe(false)
  })

  it('hides the frame when the pane is too small', () => {
    writeConversationRect(document.body, { left: 0, top: 0, width: 32, height: 32 })
    expect(document.body.hasAttribute('data-raiden-frame-hidden')).toBe(true)
  })
  ...
  it('marks compact when the pane is narrower than 420px', () => {
    ...
    expect(document.body.hasAttribute('data-raiden-frame-compact')).toBe(true)
  })
  ...
  it('updates after ResizeObserver and restores on dispose', () => {
    ...
    document.body.style.setProperty('--raiden-conversation-width', '111px')
    const center = document.createElement('div')
    ...
    expect(document.body.style.getPropertyValue('--raiden-conversation-left')).toBe('240px')
    expect(document.body.style.getPropertyValue('--raiden-conversation-width')).toBe('800px')

    mockRect(center, { left: 96, top: 8, width: 500, height: 600 })
    resizeCb?.([], {} as ResizeObserver)
    expect(document.body.style.getPropertyValue('--raiden-conversation-left')).toBe('96px')
    expect(document.body.style.getPropertyValue('--raiden-conversation-width')).toBe('500px')

    dispose()
    expect(document.body.style.getPropertyValue('--raiden-conversation-width')).toBe('111px')
    expect(document.body.hasAttribute('data-raiden-frame-hidden')).toBe(false)
  })
  ...
  it('rebinds after the center column is replaced', () => {
    ...
    expect(document.body.style.getPropertyValue('--raiden-conversation-left')).toBe('120px')
    dispose()
  })
  ...
  it('clearConversationRect removes metric variables', () => {
    ...
    expect(document.body.style.getPropertyValue('--raiden-conversation-left')).toBe('')
    expect(document.body.style.getPropertyValue('--raiden-conversation-content-left')).toBe('')
    expect(document.body.style.getPropertyValue('--raiden-frame-left')).toBe('')
  })
  ...
  it('frames the chat-flow width and the scroll viewport height, not the tall flow', () => {
    ...
    const dispose = startConversationMetrics(document, document.body)
    expect(document.body.style.getPropertyValue('--raiden-conversation-left')).toBe('280px')
    expect(document.body.style.getPropertyValue('--raiden-conversation-width')).toBe('900px')
    expect(document.body.style.getPropertyValue('--raiden-conversation-content-left')).toBe('360px')
    expect(document.body.style.getPropertyValue('--raiden-conversation-content-width')).toBe('720px')
    expect(document.body.style.getPropertyValue('--raiden-conversation-viewport-top')).toBe('48px')
    expect(document.body.style.getPropertyValue('--raiden-conversation-viewport-height')).toBe('640px')
    expect(document.body.style.getPropertyValue('--raiden-content-left')).toBe('360px')
    expect(document.body.style.getPropertyValue('--raiden-content-width')).toBe('720px')
    expect(document.body.style.getPropertyValue('--raiden-viewport-top')).toBe('48px')
    expect(document.body.style.getPropertyValue('--raiden-viewport-height')).toBe('640px')
    const frameLeft = Number.parseInt(document.body.style.getPropertyValue('--raiden-frame-left'), 10)
    const frameWidth = Number.parseInt(document.body.style.getPropertyValue('--raiden-frame-width'), 10)
    const frameTop = Number.parseInt(document.body.style.getPropertyValue('--raiden-frame-top'), 10)
    const frameHeight = Number.parseInt(document.body.style.getPropertyValue('--raiden-frame-height'), 10)
