...
describe('plugin isolation', () => {
  ...
  it('does not hide the sidebar mascot and uses distinct character attrs', () => {
    ...
    expect(mount).toContain("dataset.raidenCharacter = character")
    expect(mount).toContain("dataset.raidenMascot = 'sidebar'")
    expect(mount).not.toContain("sidebar-mascot")
  })

  it('registers the same package id on host, client, and style node', () => {
    expect(packageJson.name).toBe('@dsh-external/dsh-raiden-theme')
    expect(hostSrc).toContain("export const name = '@dsh-external/dsh-raiden-theme'")
    expect(clientSrc).toContain("export const name = '@dsh-external/dsh-raiden-theme'")
    expect(STYLE_ID).toBe('dsh-raiden-theme-style')
    expect(patch).toContain("name: '@dsh-external/dsh-raiden-theme'")
  ...
  it('does not delete foreign chrome on dispose', () => {
    ...
    owned.setAttribute('data-skin-owner', 'dsh-raiden-theme')
  ...
  it('HMR reuses a single style node', () => {
    ...
    ensureStyleNode(doc)
    ensureStyleNode(doc)
    expect(doc.querySelectorAll('#dsh-raiden-theme-style')).toHaveLength(1)
  })
  ...
  it('leaves undeclared plugin roots unmarked', () => {
    ...
    expect(plugin.hasAttribute('data-raiden-surface')).toBe(false)
    dispose()
    plugin.remove()
  })
...
describe('workspace boundaries', () => {
  it('does not contain Raiden hooks in DSH core source files', () => {
    ...
    for (const file of files) {
      ...
      expect(text, file).not.toMatch(/dsh-raiden-theme|data-raiden-/)
    }
  })
})
